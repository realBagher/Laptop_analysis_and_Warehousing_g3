...
class Scraping():
    def __init__(self):
        ...
        self.Stores={'product_id':[],'site':[],'type':[],'shop_name':[],'shop_city':[],'shop_loc':[],'date':[],'price':[]}
    ...
    def get_torob_links(self,test,category=99):
        ...
        while True:
            try:
                link=driver.find_element_by_xpath(f'//*[@id="layout-wrapp"]/div[2]/div/div[2]/div[2]/div[3]/div/div/div[{i}]/a').get_attribute('href')[:57]
                link=link.split('/')[4]
    ...
    def get_pricechart_torob(self,urls):
        ...
        async def get_each_laptop_from_torob0(id, json):
            ...
            for i in range(len(labels)):
                ...
                except:self.PricChart['meanprice'].append(None)
                try:
                    self.PricChart['lowprice'].append(json['dataSets'][1]['entries'][i]['val'])
                except:self.PricChart['lowprice'].append(None)
                try:
                    self.PricChart['product_id'].append(id)
                except:self.PricChart['product_id'].append(None)
                try:
                    self.PricChart['date'].append(labels[i])
                except:self.PricChart['date'].append(None)
    ...
    def get_data_torob(self,test,category=99):
        async def get_each_laptop_from_torob(id, json):
            ...
            except:self.Manufacturer.append(None)
            try:
                self.ManufacturerId.append(json['breadcrumbs'][3]['brand_id'])
            except:self.ManufacturerId.append(None)
            # try:
            ...
            except: self.StockStatus.append(None)
            try:
                m=json['min_price']
                self.Minprice.append(m)
            except: self.Minprice.append(None)
            try:
                m=json['max_price']
                self.Maxprice.append(m)
            except: self.Maxprice.append(None)
            # YearOfIntroduction=''
            # Category3=''
            ...
            for value in json['products_info']['result']:
                ...
                try:shop_name=value['shop_name']
                except:shop_name=None
                try:shop_city=value['shop_name2']
                except:shop_city=None
                try:shop_loc=value['page_url']
                except:shop_loc=None
                try:date=datetime.now()
                except:date=datetime.now()
                try:price=value['price']
                except:price=None
                try:active=value['price_text_mode']
                except: active=None
                if active=='active':
    ...
    def get_data_digi(self,test,category='notebook-netbook-ultrabook'):
        async def get_each_laptop_from_digi(id, json):
            ...
            try:
                ...
                self.StockStatus.append(m)
            except: self.StockStatus.append('new')
            try:
